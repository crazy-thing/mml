...
import { close, dots, folderIcon } from '../assets/exports';
import PreloadImages from '../components/PreloadImages';
import ScreenshotViewer from '../components/ScreenshotViewer';
import Switch from '../components/Switch';

const MainPanel = ({ modpack, fetchData, noChange, handleSetNoChange, animationClass, handleAnimationEnd, handleInAnimationEnd, showScroll, style, selectedModpackId }) => {
  ...
  return (
    <div className='main-panel' style={style} onClick={() => setShowUninstall(false)} onScroll={(e) => handleScroll(e)}>
      {showDel && (
      ...
      {showScrollView && (
        <div className={`main-panel-scroll-view ${showScrollViewAni ? "active" : "inactive"}`}>
          <img className='main-panel-scroll-view-thumbnail' src={`https://minecraftmigos.me/uploads/thumbnails/${modpack.thumbnail}`} />
          <p className='main-panel-scroll-view-name'> {modpack.name} </p>
          <div className='main-panel-content-top-right-buttons-button'>
                {modpack.mainVersion.id != "null" ? (
                  gameRunning ? (
                    <Button
                      onClick={handleStopGame}
                      text={"RUNNING"}
                      style={stopScrollBtnStyle}
                      textStyle={{ color: "#fff", fontSize: "2.5vh"}}
                      icon={close}
                      small={true}
                    />
                  ) : (
                    showUninstall ? (
                      <Button
                        onClick={(e) => handleDeleteModpack(e)}
                        text={"UNINSTALL"}
                        style={uninstallScrollBtnStyle}
                        textStyle={{ color: "#fff", fontSize: "2.5vh", }}
                      />
                    ) : (
                      <Button
                        onClick={installedVersion ? isInstalling ? console.log("already installing") : () => handleLaunch() : isInstalling ? console.log('already installing') : switchActive ? () => handleInstallModpack(modpack, true) : () => handleInstallModpack(modpack)}
                        text={isInstalling ? `${buttonText}%` : installedVersion ? "PLAY" : update ? "UPDATE" : "INSTALL"}
                        style={installedVersion ? isInstalling ? installScrollBtnStyle : installedScrollBtnStyle : update ? installedScrollBtnStyle : installScrollBtnStyle}
                        textStyle={{ color: "#fff", fontSize: "2.5vh",}}
                  ...
                  {installedVersion ? !gameRunning && !isInstalling && (
                    <div className='main-panel-content-top-right-buttons-button-dropdown-scroll'
                         style={isInstalling ? showUninstall ?  {background: "#f75252", border: "2px solid #f75252"} : {background: "#66b36b", border: "2px solid #66b36b"} : showUninstall ? {background: "#f75252"} : {background: "#66b36b"}} 
                         onClick={(e) => toggleUninstall(e)}
                    >
                      <img src={dots} className='main-panel-content-top-right-buttons-dropdown-img-scroll'/>
                    </div>
                  ) : null}
      ...
      {modpack && (
        <div
          className={`main-panel-content ${animationClass}`}
          onAnimationEnd={animationClass.startsWith('out') ? () => { handleAnimationEnd(); setRenderContent(false); } : () => { handleInAnimationEnd(); setRenderContent(true); }}
        >
          <div className='main-panel-content-top'>
          <div className='main-panel-content-top-thumbnail-container'>
              <img className='main-panel-content-top-thumbnail' src={`https://minecraftmigos.me/uploads/thumbnails/${modpack.thumbnail}`} />
            </div>
            ...
            <div className='main-panel-content-top-right'>
              ...
              <div className='main-panel-content-top-right-buttons'>
                ...
                {modpack.mainVersion.id != "null" ? (
                  gameRunning ? (
                    ...
                    showUninstall ? (
                      ...
                      <Button
                        ...
                        text={isInstalling ? `${buttonText}%` : installedVersion ? "PLAY" : update ? "UPDATE" : "INSTALL"}
                        style={installedVersion ? isInstalling ? installBtnStyle : installedBtnStyle : update ? installedBtnStyle : installBtnStyle}
                        textStyle={{ color: "#fff", fontSize: "3vh", }}
                        handleProg={handleProg}
                        progress={progress}
                      />     
                  ...
                  {installedVersion ? !gameRunning && !isInstalling && !showLiteUpdate &&  (
                    <div className='main-panel-content-top-right-buttons-button-dropdown'
                         style={isInstalling ? showUninstall ? {background: "#f75252", border: "4px solid #f75252"} : {background: "#66b36b", border: "4px solid #66b36b"} : showUninstall ? {background: "#f75252"} : {background: "#66b36b"}} 
                         onClick={(e) => toggleUninstall(e)}
                    >
                      <img src={dots} className='main-panel-content-top-right-buttons-dropdown-img'/>
                    </div>
                  ) : null}
                ...
                <div className='main-panel-content-top-right-buttons-wrapper'>
                  {modpack.mainVersion.liteZip != "" && !isInstalling && installedVersion && (
                    <Switch handleInstallModpack={handleInstallModpack} modpack={modpack} isActive={switchActive}
                            setIsActive={setSwitchActive} update={update} setUpdate={setUpdate}
                            setInstalledVersion={setInstalledVersion} setShowLiteUpdate={setShowLiteUpdate} />
                  )}
          ...
          {selectedItem && (
            <div className={`main-panel-content-rendered-item ${showScroll ? "active" : ""}`}>
              {renderInfo()}
